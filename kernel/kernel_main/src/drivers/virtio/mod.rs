...
/// This struct iterates over each descriptor in the chain
/// and generates a slice out of it.
/// It also frees the descriptors.
#[derive(Copy, Clone)]
...
impl<'a> SplitVirtqueueDescriptorChainIterator<'a> {
    pub fn get_pointed_chain(&self) -> Option<u16> {
        self.pointed_chain
    }
}
...
impl SplitVirtqueue {
    ...
    #[inline]
    fn descriptor_table_offset(_size: &u16) -> usize {
        0usize
    }
    #[inline]
    ...
    pub fn new_descriptor_from_unsafe_slice_mut(
        &mut self,
        buffer: UnsafeSliceMut<u8>,
        device_writable: bool,
        chain: Option<u16>,
    ) -> u16 {
        unsafe {
...
impl VirtioDevice {
    ...
    pub fn next_changed_used_ring_queue(&mut self) -> Option<u16> {
        ...
        if let Some((index, change_used_index_to)) = 'block: {
            for (idx, (driver_used_ring_index, size, _align)) in self.queue_used_sizes_align.iter()
            {
