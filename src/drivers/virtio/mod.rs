...
impl SplitVirtqueue {
	...
	fn new(size: u16) -> SplitVirtqueue {
		...
		let pointer = unsafe {
			ALLOCATOR.alloc(Layout::from_size_align(mem_size, 4096).unwrap())
		};
	...
	fn get_descriptor(&self, index: u16) -> &VirtqueueDescriptor {
		...
		unsafe {
			(self.pointer.add(Self::descriptor_table_offset(&self.size)) as *const VirtqueueDescriptor).add(index as usize).as_ref().unwrap()
		}
	}
	...
	fn get_descriptor_mut(&mut self, index: u16) -> &mut VirtqueueDescriptor {
		...
		// TODO This breaks aliasing rules if it's called twice!
		unsafe {
			(self.pointer.add(Self::descriptor_table_offset(&self.size)) as *mut VirtqueueDescriptor).add(index as usize).as_mut().unwrap()
		}
	}
	...
	pub unsafe fn new_descriptor_from_address(&mut self, address: *const (), size: usize, device_writable: bool, chain: Option<u16>) -> u16 {
	...
	pub fn new_descriptor_from_static_buffer(&mut self, buffer: &'static [u8], device_writable: bool, chain: Option<u16>) -> u16 {
		unsafe { self.new_descriptor_from_address(buffer.as_ptr() as _, buffer.len(), device_writable, chain) }
	}
	
	pub fn new_descriptor_from_static_buffer_mut(&mut self, buffer: &'static mut [u8], chain: Option<u16>) -> u16 {
		unsafe { self.new_descriptor_from_address(buffer.as_ptr() as _, buffer.len(), false, chain) }
	}
	
	pub fn new_descriptor_from_sized<T: Sized>(&mut self, buffer: &T, device_writable: bool, chain: Option<u16>) -> u16 {
		unsafe { self.new_descriptor_from_address(buffer as *const T as _, core::mem::size_of_val(buffer), device_writable, chain) }
	}
	
	/// Note that this leaks "buffer"
	/// Whoever is using this needs to make sure to run Box::from_raw on the Buffer when needed
	pub fn new_descriptor_from_boxed_slice(&mut self, buffer: Box<[u8]>, device_writable: bool, chain: Option<u16>) -> u16 {
		let len = buffer.len();
		unsafe { self.new_descriptor_from_address(Box::into_raw(buffer) as _, len, device_writable, chain) }
	}
	...
	pub fn get_available_ring_ptr(&mut self, index: u16) -> *mut u16 {
		unsafe {
			use core::ops::Add;
			(self.pointer.add(Self::available_ring_offset(&self.size)) as *mut u16).add(2).add(index as usize)
		}
	}
...
impl VirtioDevice {
	...
	pub fn configure(&mut self) {
		unsafe { 
			...
			(*self.configuration).status.write((*self.configuration).status.read() | StatusField::Acknowledge as u32);
			(*self.configuration).status.write((*self.configuration).status.read() | StatusField::Driver as u32);
	...
	pub fn accept_features(&mut self) -> Result<(), ()> {
		unsafe {
			(*self.configuration).status.write((*self.configuration).status.read() | (StatusField::FeaturesOk as u32));
