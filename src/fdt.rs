	...
	pub fn get<'this>(&'this self, path: &'this str) -> Option<&'this Node> {
		...
		if first_component.contains('@') {
			...
			if address == "" {
				...
				child = self.children.get(name)?.get(&Some(usize::from_str_radix(address, 16).unwrap_or(0)))?;
			}
	...
	fn guess(value: &'data[u8], name: Option<&str>) -> PropertyValue<'data> {
		...
		if value.len() == 4 {
			if name.unwrap_or("") == "interrupt-parent" {
				return PropertyValue::PHandleRaw(unsafe { *(value.as_ptr() as *const u32)}.swap_bytes());
			}
	...
	loop {
		match StructureToken::from(unsafe { (*token).swap_bytes() })  {
			StructureToken::BeginNode => {
				...
				unsafe { token = (token as *const u8).add(name.to_bytes_with_nul().len() + 1) as *const u32 };
			...
			StructureToken::Prop => {
				...
				if let Some(ref mut t) = current_node {
					t.properties.insert(
						name,
						PropertyValue::guess(unsafe { core::slice::from_raw_parts(token as *const u8, len) }, Some(name)),
					);
				}
