...
use crate::drivers::traits::block::{AnyBlockDevice, AnyRequestFuture, GenericBlockDevice};
use alloc::boxed::Box;
...
impl Ext2 {
	...
	async fn load_superblock(&self) -> Result<(), ()> {
		...
		let superblock: Box<Superblock> = unsafe { 
			Box::from_raw(Box::into_raw(superblock) as *mut Superblock)
		};
