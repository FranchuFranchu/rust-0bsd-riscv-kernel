use super::{code::{Ext2, Ext2Error, Result}, structures::Inode};
use alloc::boxed::Box;
use kernel_io::{Write, Read};
...
impl InodeHandleState {
    ...
    async fn write(&mut self, fs: &Ext2, source_buffer: &[u8]) -> Result<usize> {
        let block_size: usize = fs.block_size() as usize;
        fs.truncate_inode(&mut self.inode, (self.position + source_buffer.len()) as u32).await?;
        let mut position_in_buffer = 0;
        ...
        while position_in_buffer < source_buffer.len() && self.position < self.inode.size as usize {
            ...
            fs.write_inode_block_cache(&self.inode, current_block, &block).await?;
