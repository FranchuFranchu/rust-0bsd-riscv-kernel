...
use gimli::{BaseAddresses, CallFrameInstruction, CfaRule, EhFrame, FrameDescriptionEntry, LittleEndian, NativeEndian, RegisterRule, UnwindContext, UnwindSection};
...
impl<'a> From<FrameDescriptionEntry<gimli::EndianSlice<'a, NativeEndian>>> for FdeSortedByAddress<'a> {
...
pub fn row_finder(address: usize) {
...
pub fn backtrace() {
    ...
    while let Some(entry) = iter.next().unwrap() {
        match entry {
            ...
            gimli::CieOrFde::Fde(fde) => {
                let fde = fde.parse(|section, ba, offset| {
    ...
    fn constrain<'c, F>(a: &EndianSlice<'c, NativeEndian>, f: F) -> F
    where
        F: for<'a, 'b> Fn(u64, &'a mut UnwindContext<EndianSlice<'c, NativeEndian>>) -> core::result::Result<&'a UnwindTableRow<EndianSlice<'c, gimli::LittleEndian>>, gimli::Error>,
    {
        f
    }

    let fde_list = fde_list.into_sorted_vec();
    let find_row_for_address = constrain(&endian_slice, |addr: u64, ctx: &mut UnwindContext<EndianSlice<NativeEndian>>| {
        let index = fde_list.binary_search(&addr.into()).unwrap_or_else(|a| a - 1);
    ...
    while true {
        let row = find_row_for_address(frame.general_registers[Registers::Ra as usize] as u64, &mut ctx).unwrap();
        for &(register, ref rule) in row.registers() {
            println!("{:?} {:?} {:?}", row.cfa(), register.0, rule);
            
            let new_cfa = if let CfaRule::RegisterAndOffset { register: cfa_register, offset } = row.cfa() {
                frame.general_registers[cfa_register.0 as usize].wrapping_add(*offset as isize as usize)
            } else {
                unimplemented!();
            };
        ...
        println!("{:?} {:x}", "Addr", frame.general_registers[Registers::Ra as usize]);
